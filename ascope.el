...
;; Changes by Bruce Ide (FlyingRhenquest@gmail.com)
;; 1) Modified init to add "/" to init dir if it doesn't exist (init
;; fails otherwise.)
;;
;; 2) Add ascope-chunk-callback-hook which is run during ascope_process_one_chunk
;; with the function name, file and line number of each found function.
;; 3) Add ascope-make-list, which creates a list with the return data
;; from the ascope call.
;; 4) Add ascope-trace-back and supporting functions. This traces all
;; the callers of a specific function back to the top level of the
;; program, which would be handy for determining what you have to
;; recompile if you change that function.
...
(defvar ascope-chunk-callback-hook nil
"* Hook run after ascope_process_one_chunk has function, file and line number. Hoook function is passed function, file and line number.")
...
; This thing fails if the target directory doesn't end in "/"
...
; Makes a list out of an ascope query. List is in the format
; ("symbol" ((symbol file line-number) (symbol file line-number) ...))
(defun ascope-make-list-with (ascope-function symbol)
  (let (
	(function-list '())
	(function-sublist '())
	(retval (make-list 1 symbol))
	(ascope-chunk-callback-hook nil)
	(ascope-chunk-callback-hook (lambda 
					    (function-name file-name line-number)
					    (setq function-sublist '())
					    (add-to-list 'function-sublist function-name t)
					    (add-to-list 'function-sublist file-name t)
					    (add-to-list 'function-sublist line-number t)
					    (add-to-list 'function-list function-sublist t)
					    ))
	)
    (funcall ascope-function symbol)
    (add-to-list 'retval function-list t)
    retval
    )
)

(defun ascope-pretty-print-trace-list (list level buffer)
  "Pretty print a trace list"
  (set-buffer buffer)
  (insert-char 32 level)
  (insert (car list))
  (let (
	(working-list (nth 1 list))
	(one-element nil)
	)
    (if (listp working-list)
	; then
	(while (setq one-element (pop working-list))
	  (progn
	    (insert "\n")
	    (ascope-pretty-print-trace-list one-element (+ level 3) buffer)
	    )
	  ) ; end while
      ; else      
      (insert " " (nth 1 list))
...
(defun ascope-trace-back (symbol)
  "Pretty print the output of ascope-trace-back"
  (interactive (ascope-interactive "Find this symbol: "))
  (let ((indent-level 1)
	(working-buffer nil)
	(trace-list (ascope-trace-back-worker symbol))
	)
    (if (get-buffer "*ascope-trace*") (kill-buffer (get-buffer "*ascope-trace*")))
    (setq working-buffer (get-buffer-create "*ascope-trace*"))
    (set-buffer working-buffer)
    (insert "Function call trace: ")
    (insert symbol)
    (insert "\n")
    (if (not (> (safe-length trace-list) 0))
	; then
	(insert "Nothing found\n")
      ; else
      (ascope-pretty-print-trace-list trace-list indent-level working-buffer)
      )
...
; trace a function by everything that calls it, back to the top level.
; This should be useful for determining what needs to be recompiled
; if you change that function

(defun ascope-trace-back-worker (symbol &optional file)
  "Trace a function by everything that calls it, back to the top level."
  (interactive (ascope-interactive "Find this symbol: "))
  (let ((retval (make-list 1 symbol))
	(retval-list '())
	(trace-current-working-list '())
	(functions-already-handled '())
	(current-function-list nil)	
	(current-function nil)
	(current-file nil)
	(current-symbol nil)
	)
    (setq trace-current-working-list (ascope-make-list-with 'ascope-find-functions-calling-this-function symbol))
    (setq current-function-list (cadr trace-current-working-list))
    (if (> (safe-length current-function-list) 0)
	; then
	(while (setq current-function (pop current-function-list))
	  (setq current-symbol (pop current-function))
	  (setq current-file (pop current-function))
	  (add-to-list 'functions-already-handled current-symbol t)
	  (add-to-list 'retval-list (ascope-trace-back-worker current-symbol current-file) t)      
	  )
      ; else
      (add-to-list 'retval file t)
      )
    (if (> (safe-length retval-list) 0)
	(add-to-list 'retval retval-list t))
    retval
    )
)
...
(run-hook-with-args 'ascope-chunk-callback-hook function-name file line-number)
